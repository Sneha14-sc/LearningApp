...
const collectionVer2 = require('./collection.model');
...
//TODO: set acceptable findFields
...
router.get('/countTotal', function (req, res, next) {
    ...
    collectionVer2.count({}, function (err, count) {
        if (err) {
...
router.get('/', function (req, res, next) {
    console.log(path.basename(__filename), 'getAPI(req.query) ' + JSON.stringify(req.query));
    ...
    if(apiParamMng.checkGetQueryParams(req.query, getAPI__acceptableGetQueryParams)) {
        ...
        collectionVer2.find(findObject)
            // .select(selectObject)
            // .limit(limitValue)
            // .skip(skipValue)
            // .sort(sortObject)
...
router.post('/', function (req, res,next) {
    ...
    if(_.isEmpty(postInfo)) {
        ...
        collectionVer2.create(postInfo, function (err, doc) {
            if (err) {
                res.status(status.INTERNAL_SERVER_ERROR).json({"error": "something is wrong"});
            } else {
...
router.post('/:id/edit', function(req, res, next){
    ...
    if(apiParamMng.checkGetQueryParams(req.body, editAPI__acceptableEditParams)) {
        collectionVer2.findOneAndUpdate({_id: req.params.id}, {$set: req.body})
            .exec(function (err) {
                if (err) {
                    res.status(status.INTERNAL_SERVER_ERROR).json({"error": "something is wrong"});
                } else {
